...
function resolveDirectSshAuthority(
  ...
  if (!state.remoteWorkspaceHydratedTargetIds?.has(targetId)) {
    // Not connected, still pulling, or timed out — all are "we could not ask", never "nothing there".
    return 'unverifiable'
  }
  // Why: the same pair use-app-session-persistence.ts gates uploads on. A conflicting snapshot means
  // the client's picture is not the host's, so it is no basis for deciding the host holds nothing.
  return state.remoteWorkspaceSyncStatusByTargetId?.[targetId]?.phase === 'conflict'
    ? 'unverifiable'
    : 'none'
}
