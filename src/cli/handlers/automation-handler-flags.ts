...
const PRESET_TRIGGERS: Record<Exclude<AutomationSchedulePreset, 'custom'>, true> = {
  hourly: true,
  daily: true,
  weekdays: true,
  weekly: true
...
function validateScheduleModifierApplicability(
  ...
  const isPreset = PRESET_TRIGGERS[raw as Exclude<AutomationSchedulePreset, 'custom'>]
  if (!isPreset) {
    if (flags.has('time')) {
...
export function getScheduleFlag(
  ...
  const rrule = PRESET_TRIGGERS[raw as Exclude<AutomationSchedulePreset, 'custom'>]
