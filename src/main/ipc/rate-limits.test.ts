...
describe('registerRateLimitHandlers', () => {
  ...
  it('keeps the existing rate-limit channels registered', () => {
    ...
    expect(ipcState.handleHandlers.has('rateLimits:refreshAntigravity')).toBe(true)
  })
  ...
  it('registers a Grok reset consume channel that delegates to RateLimitService', async () => {
