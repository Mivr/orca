...
describe('RateLimitService Antigravity usage', () => {
  ...
  it('keeps Gemini failures off the Antigravity provider', async () => {
    ...
    vi.mocked(fetchAntigravityRateLimits).mockResolvedValue(
      errorProvider('antigravity', 'Antigravity is not signed in on this computer.')
    )
  ...
  it('publishes native Antigravity usage independently of Gemini', async () => {
    vi.mocked(fetchGeminiRateLimits).mockResolvedValue(
      errorProvider('gemini', 'Gemini project ID not found')
    )
    vi.mocked(fetchAntigravityRateLimits).mockResolvedValue(
