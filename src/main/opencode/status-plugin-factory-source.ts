...
export function getStatusPluginFactorySource(options: {
  ...
  return [
    ...
    '',
    '// Why: OpenCode resolves plugins through the module default export. The v1',
    '// loader rejects the module unless the default exposes `server()`, while the',
    '// v2 loader (plain `opencode` since 2.0) rejects it unless the default exposes',
    '// `effect` or `setup` — a server()-only export fails v2 with "Missing key at',
    '// [default][effect] / [default][setup]". Keep both so either loader accepts',
    '// the module, plus the named export so the factory-based loader still finds',
    '// the same instance.',
    'export default {',
    `  id: "${pluginID}",`,
