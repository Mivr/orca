...
test.describe('SSH cold hydration gap tab seeding', () => {
  ...
  test('adds no tab when a client with no local row connects to a host that already owns tabs', async (// oxlint-disable-next-line no-empty-pattern -- This restart test owns every Electron launch.
  {}, testInfo) => {
    test.setTimeout(600_000)
