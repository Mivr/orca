...
//   ssh-docker-relay-perf.spec.ts — wall-clock latency thresholds belong to the perf lane
//     (test:e2e:ssh-docker-perf); flaky budgets here would cost the lane its credibility.
//   ssh-codex-display-artifacts-repro.spec.ts — installs a real remote codex binary that CI
//     runners do not have (observed as `spawn codex ENOENT`).
//
