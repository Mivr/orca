...
describe('PR E2E gate contract', () => {
  ...
  it('gives every Docker-gated SSH spec a lane that runs it', () => {
    ...
    ].map((file) => readFileSync(join(projectDir, 'config/scripts', file), 'utf8'))

    // Why the const form and not the bare name: preview and demo specs cite the flag in a
    // "how to run me" comment without gating on it.
    const dockerGatedSpecs = readdirSync(join(projectDir, 'tests/e2e'))
      .filter((file) => file.endsWith('.spec.ts'))
      .map((file) => `tests/e2e/${file}`)
      .filter((spec) =>
        readFileSync(join(projectDir, spec), 'utf8').includes("ORCA_E2E_SSH_DOCKER === '1'")
      )
